...
If you want to avoid building large CUDA images locally on macOS, use the
included GitHub Actions workflow at `.github/workflows/build-docker-images.yml`.
...
5. Choose a target such as `cu124-py311` and start the workflow.

The workflow logs in to `ghcr.io` using the repository's GitHub token and
pushes images to `ghcr.io/<repo-owner>/comfyui`.
